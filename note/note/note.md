...
推后执行的任务不需要睡眠，选择软中断或tasklet
工作队列是唯一能在进程上下文中运行的下半部实现机制，也只有它可以睡眠
...
```bash
- `malloc` 是用户态通用分配函数，只保证虚拟连续，不清零；
- `kzalloc` 是内核态小内存分配，物理 + 虚拟均连续且清零，速度快，支持原子态分配；
- `vmalloc` 是内核态大内存分配，仅虚拟连续，速度慢，仅能在进程上下文使用。
```

## malloc和new

```bash
malloc和new都可以在堆上分配空间，区别是
malloc需要指定分配空间的大小，new只需要分配空间的数据类型
malloc返回的是指向分配内存空间的起始地址，通常需要强制类型转换；而new返回的是指向分配空间的指针
malloc分配的内存空间不需要初始化，返回的是一段未初始化的内存区域，new分配的内存空间需要进行初始化
```