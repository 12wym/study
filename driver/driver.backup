![[f6bbde19dbd558c3781d43323a06ce80 2.jpeg]]

/bin : (二进制可执行文件)存放系统最基本的**用户命令**（所有用户可用）
- **示例**：`ls`、`cp`、`mv`、`bash` 等
- **特点**
    - 通常是软链接到 `/usr/bin`（现代 Linux 发行版）
    - 系统启动或修复时必需的命令
/boot: **(启动文件) 存放内核文件（vmlinuz）**、**引导加载器（GRUB）** 和 **初始化内存盘（initramfs）**
- **内容**
    - `vmlinuz-*`：Linux 内核文件
    - `initrd.img-*`：临时根文件系统（用于早期用户空间）
    - `grub/`：GRUB 引导配置
/dev: (设备文件)包含 **设备文件**（硬件或虚拟设备的接口）
- **示例**
    - `/dev/sda`：第一块 SATA 硬盘
    - `/dev/ttyUSB0`：USB 串口设备
    - `/dev/null`：黑洞设备（丢弃所有写入数据）
/etc: (系统配置文件)存放 **全局配置文件**（系统和应用程序的配置）
- **重要文件/目录**
    - `/etc/passwd`：用户账户信息
    - `/etc/fstab`：文件系统挂载表
    - `/etc/network/`：网络配置
/home: (用户主目录)普通用户的**个人文件和配置**存储位置
- **示例**
    - `~/.bashrc`：用户的 Bash 配置文件
    - `~/Documents`：用户文档
/lib,lib64: (共享库)存放系统**核心共享库**（`.so` 文件）和内核模块
- **示例**：
    - `libc.so`：C 标准库
    - `/lib/modules/`：内核模块（驱动）
/lost+found: (恢复文件)fsck`（文件系统修复工具）存放**恢复的碎片文件**
- **注意**：仅 `ext3/ext4` 文件系统会有此目录
/media: (可移动设备挂载点)自动挂载 **U 盘、光盘** 等可移动设备
- **示例
    - `/media/usb`：U 盘挂载点
/mnt: (临时挂载点)管理员手动挂载文件系统的临时目录
- **典型用途**：
    - `mount /dev/sdb1 /mnt`：挂载硬盘分区
/opt: (可选软件)存放**第三方大型软件**（如 MATLAB、Oracle）
- **特点**：软件通常自带所有依赖库
/proc: (内核和进程信息)虚拟文件系统，提供**内核和进程的实时信息**
- **重要文件**
    - `/proc/cpuinfo`：CPU 信息
    - `/proc/meminfo`：内存使用情况
    - `/proc/pid/`：特定进程的详细信息
/root: (root用户的主目录)
**作用**：超级用户（`root`）的**个人目录**（普通用户无权限访问）
/run: (运行时数据)存放**系统运行时的临时文件**（如 PID 文件、套接字）
- **示例**
    - `/run/sshd.pid`：SSH 守护进程的 PID
/sbin: (系统管理命令)存放 **系统管理员命令**（需 `root` 权限）
- **示例**
    - `fdisk`：磁盘分区工具
    - `iptables`：防火墙配置
/srv: (服务数据)存放 **服务相关的数据**（如 Web 服务器的网站文件）
- **示例**
    - `/srv/www/`：Apache/Nginx 的网站根目录
/sys: (内核设备树) 虚拟文件系统，提供**内核设备、驱动和硬件的配置接口**
- **用途**
    - 动态调整内核参数（如 `echo 1 > /sys/class/leds/trigger`）
/tmp（临时文件）
- **作用**：存放**临时文件**（重启后可能被清除）
- **特点**
...
- **作用**：存放 **用户应用程序和只读数据**（二级文件系统）
- **重要子目录**
    - `/usr/bin`：用户命令（非关键）
    - `/usr/lib`：应用程序库
    - `/usr/local`：本地编译安装的软件
/var（可变数据）
- **作用**：存放**经常变化的文件**（日志、缓存、邮件等）
- **重要子目录**
